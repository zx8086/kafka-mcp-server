...
import { z } from "zod";
import { TopicNameParam, GroupIdParam } from "../shared/parameters.ts";

export const DeleteTopicParams = z.object({
  topic: TopicNameParam.describe(
    "Name of the topic to delete. This action is irreversible."
  ),
});
...
export const ResetConsumerGroupOffsetsParams = z.object({
  ...
  timestamp: z
    ...
    .optional()
    .describe(
      "Unix timestamp in ms (required when strategy is 'timestamp')"
    ),
});
